// YOLOv8n live display + lightweight SORT-style tracking for RK3588.
//
// Usage:
//   ./rknn_yolov8_tracker_live_demo model/yolov8n_runtime152.rknn rtsp://user:pass@ip:554/stream1
...
int main(int argc, char **argv)
{
    ...
    const char *window_name = "RK3588 YOLOv8n Tracking";
    ...
    printf("YOLOv8n live tracking started on %s\n", input_arg);
    ...
    while (g_running)
    {
        ...
        if (frame_index > 0 && frame_index % 30 == 0)
        {
            ...
            printf("tracking_perf frame=%d fps=%.2f detections=%zu tracks=%zu\n",
        ...
        char status[128];
        snprintf(status, sizeof(status), "YOLOv8n Tracking  active IDs: %zu  detections: %zu",
